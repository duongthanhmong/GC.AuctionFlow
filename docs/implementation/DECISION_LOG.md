...
## D-P1E-001 — Auction Episode Observation Foundation (geometric; no Acceptance)

- **Decision:** Phase 1E implements Auction Episode Observation Foundation only (`AUCTION_EPISODE_POLICY_V1`). **Reference Excursion is not an MBO Sweep** — a trade beyond a Confirmed reference is geometric OutsideAttempt / Developing observation only; it does not prove stop hunting, trapped participants, liquidity sweep, reversal, continuation, or acceptance. **One episode per Current Primary Auction + ReferenceId**; repeated crossings update the same `EpisodeId` and increment `AttemptCount` only on transitions into the canonical outside side (boundary roles). Confirmed Previous Primary and Confirmed Composite references only — Developing / Preview / Current Primary moving levels excluded (`DEVELOPING_REFERENCE_EPISODES_NOT_AUTHORIZED`). Centerline (POC/VPOC) tracks touch/cross and side excursions without OutsideAttempt/ReentryDeveloping or inside/outside acceptance geometry. No approach distance (`APPROACH_DISTANCE_NOT_CALIBRATED` — ApproachingReference reserved, not emitted). No intra-auction calibrated reset (`INTRA_AUCTION_EPISODE_RESET_NOT_CALIBRATED`) — new episodes only on Primary Auction change, reference retirement, epoch/disable/InvalidData. No Acceptance / stable re-entry Resolution (Phase 1F). No FAR/AAC, Orderflow interpretation, Long/Short, alerts, or Episode overlay. Raw excursion metrics and LocalPoc (when exact volume) preserved for Phase 1F — Local Value left unavailable (`EPISODE_LOCAL_VALUE_POLICY_NOT_AUTHORIZED`). Historical reconstruction is **LIVE_ONLY** when exact ordered chart-load trades are unavailable. Directional Context is optional provenance only. Runtime schema **0.6.0**; assembly **0.0.6**; prior policies unchanged. Profile/Composite/Reference/Directional must not consume Episode state. Live acceptance required before commit/tag.
- **Date:** 2026-07-24

## D-P1E-002 — Episode OnNewTrades admission decoupled from TradeStreamProbe gate

- **Decision:** Live defect: GPS showed `TRADES: OBSERVED` while `EPISODES: AWAITING TRADES` with eligible Confirmed references and continuing profile/directional updates. Root cause: `OnNewTrades` gated Episode `TryProcessEpisodeTrade` behind `TradeStreamProbeGates` / `EnableTradeStreamProbe` (default OFF). Global trade observation used `NoteTradeCallback` and did not require the probe. Fix: normalize each batch print once; always admit to Episode when Episodes enabled; apply probe gates only to probe enqueue. Accepted trades mark module Ready/Partial even with zero active episodes. Dedup/order protections unchanged. No bar fabrication; no Phase 1F. Live re-test required; commit/tag remain HOLD.
- **Date:** 2026-07-24

## D-P1E-003 — Phase 1E FINAL PASS WITH DOCUMENTED LIVE STATE-MACHINE COVERAGE LIMITATION + lock

- **Decision:** Phase 1E Auction Episode Observation Foundation is **FINAL PASS WITH DOCUMENTED LIVE STATE-MACHINE COVERAGE LIMITATION** and locked. Episode Observation accepted (`AUCTION_EPISODE_POLICY_V1`). **Reference Excursion remains distinct from MBO Sweep.** Trade admission is independent from Trade Stream Probe enablement (D-P1E-002). One active episode per Primary Auction and ReferenceId; repeated boundary excursions update AttemptCount; Centerlines use CrossCount and side excursions, not AttemptCount. Confirmed references only. **LIVE_ONLY** historical policy accepted — no fabricated OHLC interactions. No intra-auction reset threshold; no approach threshold; no Acceptance/Re-entry Resolution; no FAR/AAC or signal output. Live core gate on GCQ6 / Rithmic Live: EPISODES PARTIAL; ELIGIBLE 16; ACTIVE 4; LATEST DEVELOPING; Centerline PreviousPrimaryVpoc @ 4051.2 AttemptCount 0 max excursion 60 ticks Direction DOWN; DATA DEGRADED independent; MBO BLOCKED; no prohibited wording. Documented live state-machine coverage limitation accepted (repeated-attempt / re-entry / auction expiry / retirement / disable-reenable / stale-order / revision sequences / history overlap / LocalPoc ties = automated). Tests **439**; DLL SHA-256 `924DB65C4D719D831926C81392AF600A332CD6BFF81401C5B6FC9E30CDFACBC2`. Tag `gcae-p1e-auction-episode-observation-pass`. Phase 1F requires a separate implementation boundary and authorization — **NOT STARTED**.
- **Date:** 2026-07-24

## D-P1F-001 — Acceptance / Re-entry Evidence Measurement Foundation (measurement ≠ resolution)

- **Decision:** Phase 1F implements Acceptance / Re-entry Evidence Measurement only (`ACCEPTANCE_REENTRY_EVIDENCE_POLICY_V1`). Measurement is separated from resolution. Raw Acceptance and Re-entry evidence vectors are preserved without probability interpretation. No Established Acceptance, Failed Acceptance, Stable Reacceptance, or ReentryFailed is emitted. No FAR/AAC, Thesis, Entry/Risk, or Long/Short signal. Centerline evidence is marked `CENTERLINE_ACCEPTANCE_GEOMETRY_NOT_APPLICABLE` and does not receive canonical outside/reentry geometry. LIVE_ONLY history is inherited from Phase 1E Episode admission via read-only `EpisodeMeasurementEvent` feed — no candle OHLC reconstruction. Phase 1F does not mutate Episode State, Episode Resolution, AttemptCount, Reference state, or Directional state. Unavailable optional components remain explicit null/unavailable with reason codes (OutsideCloseRatio, TPO outside, Local Value, OldValueReclaimFailure, RetestHoldQuality, opposite-aggression effectiveness). Ratios are descriptive only; zero denominator → unavailable (never NaN/Infinity). No invented acceptance, maintenance, or stable-reentry thresholds. Future Acceptance/Re-entry Resolution requires separately authorized calibrated thresholds. Runtime schema **0.7.0**; assembly **0.0.6**; prior policies unchanged. Live acceptance required before commit/tag.
- **Date:** 2026-07-24

## D-P1F-002 — Phase 1F FINAL PASS WITH DOCUMENTED LIVE EVIDENCE-LIFECYCLE COVERAGE LIMITATION + lock

- **Decision:** Phase 1F Acceptance / Re-entry Evidence Measurement Foundation is **FINAL PASS WITH DOCUMENTED LIVE EVIDENCE-LIFECYCLE COVERAGE LIMITATION** and locked. Evidence Measurement accepted (`ACCEPTANCE_REENTRY_EVIDENCE_POLICY_V1`). Measurement remains separate from resolution; Phase 1F is observer-only. EvidenceId is stable per EpisodeId. Raw evidence vectors are preserved without qualitative thresholds; ratios are descriptive, not probabilities. Episode State / Resolution / AttemptCount remain immutable to Phase 1F. Geometric re-entry is not Stable Reacceptance; Unresolved is a valid Production observation state. Centerline canonical acceptance geometry remains not applicable. LIVE_ONLY history accepted — no candle reconstruction. No Established Acceptance; no StableReaccepted/ReentryFailed; no FAR/AAC, Thesis, or signal output. Live focused core gate on GCQ6 / Rithmic Live: Evidence PARTIAL; ACTIVE EVIDENCE SETS 2; UpperBoundary PreviousPrimaryTpoVah @ 4063.9; ACCEPTANCE OBS UNRESOLVED; REENTRY OBS GEOMETRICREENTRY; outside ratios time 0.4242 / volume 0.4479 / trade 0.4404; Local POC displacement −8 ticks; Episode remained REENTRYDEVELOPING AttemptCount 1 max excursion 29 ticks; DATA DEGRADED independent; MBO BLOCKED. Documented live evidence-lifecycle coverage limitation accepted (segmentation / LowerBoundary / zero-denominator / revisions / lifecycle / Centerline / bid-ask variants / fail-closed / replay = automated). Tests **458**; DLL SHA-256 `1FDEBBB3E4E94497157FF6FA7D621760028AA516497BAB8800D88CF5C9E07249`. Tag `gcae-p1f-acceptance-reentry-evidence-measurement-pass`. Future Acceptance/Re-entry Resolution, FAR/AAC, and Phase 2 Executed Orderflow require separate calibrated boundaries — **NOT STARTED**.
- **Date:** 2026-07-24

## D-P2A-001 — Executed Orderflow Raw Feature Foundation (raw ≠ interpretation)

- **Decision:** Phase 2A implements Executed Orderflow raw features only (`EXECUTED_ORDERFLOW_POLICY_V1`). Raw executed Orderflow is separated from interpretation. One authoritative normalized executed-trade event from `TradeStreamAtasMapper.MapNewTrade` / `NewTradeObservation`; Trade Stream Probe, Recorder, and MBO do not gate raw Orderflow. Cumulative callbacks are not authoritative for executed totals (`CUMULATIVE_CALLBACKS_NOT_AUTHORITATIVE_FOR_EXECUTED_TOTALS`). Unknown aggressor volume remains explicit. Partial Delta is labeled Classified Delta. CVD is cumulative classified executed aggression within the observation scope only — not positions, institutional flow, or covering. No position-intent claims. No imbalance / Big Trade / tape-speed / absorption / exhaustion thresholds. No Effort vs Result or Trade Facilitation. LIVE_ONLY coverage (MidAuction or FromAuctionStart). Runtime schema **0.8.0**; assembly **0.0.6**; prior policies unchanged. Phase 2B Trade Facilitation and Resolution/FAR/AAC require separate implementation boundaries — **NOT STARTED**. Live acceptance required before commit/tag.
- **Date:** 2026-07-24

## D-P2A-002 — Phase 2A raw executed Orderflow accepted and locked

- **Decision:** Phase 2A Executed Orderflow Raw Feature Foundation is **FINAL PASS WITH DOCUMENTED LIVE RAW-FEATURE COVERAGE LIMITATION** and locked. Raw executed Orderflow accepted (`EXECUTED_ORDERFLOW_POLICY_V1`). One authoritative normalized executed-trade event path. Trade Stream Probe, Recorder, and MBO do not gate raw Orderflow. NewTrade/NewTrades overlap remains idempotent. Cumulative callbacks are not authoritative for totals. Unknown aggressor volume remains explicit; Ask/Bid are never fabricated. Partial Delta is labeled Classified Delta. CVD means cumulative classified executed aggression only — Delta/CVD do not imply opening/closing positions or intent. LIVE_ONLY and MidAuction coverage are accepted limitations. No imbalance/Big Trade thresholds; no absorption/exhaustion; no Effort vs Result or Trade Facilitation; no Resolution/FAR/AAC/Thesis. Live focused core gate on GCQ6 / Rithmic Live: ORDERFLOW PARTIAL; POLICY EXECUTED_ORDERFLOW_POLICY_V1; COVERAGE LIVEONLYMIDAUCTION; EXECUTED VOLUME 113; TRADES 103; ASK/BID unavailable; UNKNOWN AGGRESSOR VOLUME 113; CLASSIFIED DELTA 0; CLASSIFIED CVD 0; AGGRESSOR COVERAGE 0; EPISODE ORDERFLOW PARTIAL; DATA DEGRADED independent; MBO BLOCKED; Probe OFF admission verified. Documented live raw-feature coverage limitation accepted (Ask/Bid READY paths, nonzero Delta/CVD, full price-ledger/Episode/timing/lifecycle/revision/replay = automated). Tests **469**; DLL SHA-256 `F92538852AD2478002F6FCB89B052FC9ACBD3773746F733C31548BF77B5356D2`. Tag `gcae-p2a-executed-orderflow-raw-feature-foundation-pass`. Phase 2B Cluster Raw Feature Measurement, Effort vs Result, Trade Facilitation, Resolution/FAR/AAC require separate implementation boundaries — Cluster Raw authorized separately; interpretation classifiers **NOT STARTED**.
- **Date:** 2026-07-24

## D-P2B-001 — Cluster Raw Feature Measurement Foundation (raw ≠ classification)

- **Decision:** Phase 2B implements Cluster Raw Feature Measurement only (`CLUSTER_RAW_FEATURE_POLICY_V1`). Phase 2A remains the authoritative executed-trade source; Phase 2B does not remap ATAS trades or create a second ledger. Input is the Phase 2A immutable price-level ledger plus read-only changed-tick fan-out after Orderflow acceptance (version-gated; no separate ExecutedOrderflowMutationEvent type required). Raw same-price and diagonal ratios are descriptive measurements — not imbalance classification. Neighboring RawDominantSide continuity is not stacked imbalance. EMPIRICAL_MIDRANK_V1 percentiles are descriptive — not Extreme labels or probabilities. Visits/revisits are not Repeated Extreme Tests. Unavailable Ask/Bid remain explicit (never fabricated zero). No production thresholds. No Big Trade / absorption / exhaustion / Effort vs Result / Trade Facilitation. CLUSTER CLASSIFICATION: NOT CALIBRATED. Runtime schema **0.9.0**; assembly **0.0.6**; prior policies unchanged. Future calibrated cluster classification requires GC calibration and a separate boundary. Effort vs Result, Trade Facilitation, Resolution/FAR/AAC remain **NOT STARTED**. Live acceptance required before commit/tag.
- **Date:** 2026-07-25

## D-P2B-002 — Phase 2B Cluster Raw Measurement accepted and locked

- **Decision:** Phase 2B Cluster Raw Feature Measurement Foundation is **FINAL PASS WITH DOCUMENTED LIVE CLASSIFIED-CLUSTER COVERAGE LIMITATION** and locked. Phase 2A remains the sole authoritative executed-trade source; no second normalization or trade-admission authority. Raw ratios are not Imbalance classification. Raw dominance is not buyer/seller control. Neighboring raw dominance is not Stacked Imbalance. Empirical rank/percentile is descriptive, not Extreme or probability. Visits/revisits are not Repeated Extreme Tests. Unavailable Ask/Bid remains explicit; Unknown-only input remains valid Partial. No threshold invented. ClassificationState remains NotCalibrated. No Big Trade; no Absorption/Exhaustion; no Effort vs Result; no Trade Facilitation; no Resolution/FAR/AAC/Thesis. Live focused core gate on GCQ6 / Rithmic Live (two snapshots ~2s): ORDERFLOW PARTIAL LIVEONLYFROMAUCTIONSTART volume 318→324 / trades 258→264 / Unknown 318→324; CLUSTER RAW PARTIAL levels 5→6 Unknown-only; CLASSIFICATION NOT CALIBRATED; latest tick 4071.0→4070.7; Volume Rank 4/5→6/6; Ask/Bid ratios unavailable; Abs Delta rank unavailable; Episode Cluster Raw PARTIAL; HISTORY LIVE_ONLY; DATA DEGRADED independent; MBO BLOCKED. LEVEL TRADES 90→84 is different latest ticks, not a decrement. Documented live classified-cluster coverage limitation accepted (classified Ask/Bid/diagonal/dominance/abs-Delta/ties/revisits/lifecycle/replay = automated). Tests **482** (×2) + Probe/Recorder **139**; DLL SHA-256 `A15CC6A85AA9562E59CA8B66140AAD017E957F96E96C7BBF47E620E6E0E71A39`. Tag `gcae-p2b-cluster-raw-feature-measurement-pass`. Future classification requires GC calibration and a separate implementation boundary — Imbalance/Extreme/Big Trade/Effort vs Result/Trade Facilitation/Resolution/FAR/AAC **NOT STARTED**.
- **Date:** 2026-07-25

## D-P2C-001 — Auction Efficiency Raw Evidence Measurement Foundation (measurement ≠ classification)

- **Decision:** Phase 2C separates raw Auction Efficiency evidence from classification (`AUCTION_EFFICIENCY_EVIDENCE_POLICY_V1`). Phase 2A/2B remain authoritative sources for executed totals and Cluster Raw; Phase 2C does not create a second trade or Cluster ledger and does not normalize ATAS trades. Effort and Result remain separate immutable vectors. Raw progress-per-unit measurements are descriptive only — no one-dimensional `Delta / ticks` classifier, no EfficiencyScore, no score or probability. Unknown aggressor remains explicit; Ask/Bid-dependent components stay unavailable when unclassified. No imbalance / Big Trade / MBO / Stop / Iceberg effort is fabricated. No Effective/Ineffective, Absorption/Exhaustion, or Trade Facilitation output. ClassificationState is NotCalibrated only. Immutable Profile start anchors are captured within LIVE_ONLY observation scope and must not be rewritten by developing revisions. Runtime schema **0.10.0**; assembly **0.0.6**; prior policies unchanged. Future Effort vs Result / Trade Facilitation classification requires GC regime calibration and a separate authorized boundary. Resolution/FAR/AAC remain **NOT STARTED**. Live acceptance required before commit/tag.
- **Date:** 2026-07-25

## D-P5A-002 — Narrow the depth/quote recording lock to match its own evidence

- **Decision:** Passive depth and best-bid/ask recording is authorised (`EnableDepthAndQuoteRecording`, default false). The scope lock in `CallbackGroupingAndFanOutTests` previously forbade any depth, quote or MBO recording setting on the indicator. Its stated rationale is **P0-06D**: an abnormal bar reproduced on a second GCQ6 chart that did not have the indicator attached. That was observed during an **active MBO subscription**, so the lock was written wider than the evidence supporting it — `MarketDepthChanged` and `OnBestBidAskChanged` are raised by the platform on their own, and receiving them subscribes to nothing and requests nothing. v1.2 §46.5 lists DOM changes among the data no downloaded history contains, so leaving them unwritten discards them permanently.
- **Still locked, and for the reason the evidence actually supports:** MBO recording (`MboOperationalLock.MboRecordingEnabled` stays false) and DOM snapshot pull. Both are **actions** rather than observation, and actions are what carried the risk. `TryRecordDepth` is asserted to contain no `TryRequestSnapshotPull` and no `Subscribe`; both mutations were verified to fail the guard.
- **MBO remains isolated-environment-only** per P0-06D: a separate ATAS process, VM or machine. It must not be enabled in the process used for primary GC analysis or trading, and a separate chart or workspace in the same process is not proven isolation.
- **Also in this change:** `bidAskClassificationState`, `domState` and `mboState` were hard-coded constants on the capability snapshot. They are now measured from observed callbacks, so the card reports what the feed actually delivers instead of restating a policy decision. `Blocked` and `Unavailable` are now distinguished: the first means we chose not to look, the second that we looked and it was not there.
- **Date:** 2026-07-27
