...
public sealed class AuctionGpsCardViewModel
{
    ...
    /// <summary>
    /// Short build identity, taken from the assembly's module version id.
    ///
    /// The MVID is regenerated by every compilation and is stable for a given binary, so
    /// it answers the one question a screenshot otherwise cannot: is the DLL running in
    /// ATAS the one that was just deployed. Without it a stale load is indistinguishable
    /// from a failed fix, and both look like "nothing changed".
    /// </summary>
