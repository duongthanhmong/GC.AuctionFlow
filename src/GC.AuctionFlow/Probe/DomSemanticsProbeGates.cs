...
/// <summary>Fail-closed gates for DOM semantics probe. Rejection never becomes DOM capability PASS.</summary>
public static class DomSemanticsProbeGates
{
    ...
    {
        if (!enable)
            return GateDecision.Reject(TradeStreamGateReason.ProbeDisabled, "EnableDomSemanticsProbe=false", false);
